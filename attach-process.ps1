<#
Start a process with WinDbg attached, and restart it after WinDbg exits.
Based on Ben 'epi' Risher's attach-process.ps1
https://github.com/epi052/osed-scripts/blob/main/attach-process.ps1
#>
...
process {
    ...
    $cmd_args = "-WF $HOME\Desktop\windbg\dark-green-x64.wew -p $($process.id) -c '.load pykd.pyd; !py mona config -set workingfolder $HOME\Desktop\monalogs\%p_%i;"
    ...
    Write-Host "[+] WinDbg args: $cmd_args"
